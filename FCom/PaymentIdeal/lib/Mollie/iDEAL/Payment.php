...
class Mollie_iDEAL_Payment
{
    ...
    // error constants
    const IDEAL_NO_BANK_XML = 0;
    const IDEAL_BANK_ID_FAIL = 1;
    const IDEAL_DESCR_FAIL = 2;
    const IDEAL_AMOUNT_FAIL = 3;
    const IDEAL_RETURN_URL_FAIL = 4;
    const IDEAL_REPORT_URL_FAIL = 5;
    const IDEAL_TRANS_ID_FAIL = 6;
    const IDEAL_DESCR_AMOUNT_FAIL = 7;
    const IDEAL_CONNECTION_FAIL = 8;
    const IDEAL_XML_FAIL = 9;
    const IDEAL_GENERAL_FAIL = 10;

    // error messages

    protected $error_messages = array(
        self::IDEAL_NO_BANK_XML => "No XML or XML with API error received from Mollie", // "Geen XML of XML met API fout ontvangen van Mollie"
        self::IDEAL_BANK_ID_FAIL => "The bank specified %s is incorrect or incomplete", // "De opgegeven bank %s is onjuist of incompleet"
        self::IDEAL_DESCR_FAIL => "The specified description %s is incomplete", // "De opgegeven omschrijving %s is incompleet"
        self::IDEAL_AMOUNT_FAIL => "The amount specified %s is incorrect or too low", // "Het opgegeven bedrag %s is onjuist of te laag"
        self::IDEAL_RETURN_URL_FAIL => "The return URL specified %s is incorrect", // "De opgegeven return URL %s is onjuist"
        self::IDEAL_REPORT_URL_FAIL => "The report specified URL% s is incorrect", // "De opgegeven report URL %s is onjuist"
        self::IDEAL_TRANS_ID_FAIL => "There is an incorrect transaction ID specified", // "Er is een onjuist transactie ID opgegeven"
        self::IDEAL_DESCR_AMOUNT_FAIL => "You need a description and amount (in cents) to generate iDEAL link.", // "U moet een omschrijving én bedrag (in centen) opgeven voor de iDEAL link."
        self::IDEAL_CONNECTION_FAIL => "Failed to communicate with Mollie: %s", // "Fout bij communiceren met Mollie: %s"
        self::IDEAL_XML_FAIL => "Could not process XML result", // "Kon XML resultaat niet verwerken"
        self::IDEAL_GENERAL_FAIL => "An error has occured, please try again later."
    );
    ...
    public function getBanks()
    {
        ...
        if (!$banks_object or $this->_XMlisError($banks_object)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_NO_BANK_XML]);
            return false;
        }
    ...
    public function createPayment($bank_id, $amount, $description, $return_url, $report_url)
    {
        if (!$this->setBankId($bank_id)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_BANK_ID_FAIL], array($bank_id));
            return false;
        }

        if (!$this->setDescription($description)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_DESCR_FAIL], array($description));
            return false;
        }

        if (!$this->setAmount($amount)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_AMOUNT_FAIL], array($amount));
            return false;
        }

        if (!$this->setReturnURL($return_url)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_RETURN_URL_FAIL], array($return_url));
            return false;
        }

        if (!$this->setReportURL($report_url)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_REPORT_URL_FAIL], array($report_url));
            return false;
        }
    ...
    public function checkPayment($transaction_id)
    {
        if (!$this->setTransactionId($transaction_id)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_TRANS_ID_FAIL]);
            return false;
        }
    ...
    public function CreatePaymentLink($description, $amount)
    {
        if (!$this->setDescription($description) or !$this->setAmount($amount)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_DESCR_AMOUNT_FAIL]);
            return false;
        }
    ...
    protected function _sendRequest($path, $data = '')
    {
        ...
        if (curl_error($ch)) {
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_CONNECTION_FAIL], array(curl_error($ch)));
    ...
    protected function _XMLtoObject($xml)
    {
        ...
        if (!$xml_object) {
            $this->error_code    = -2;
            $this->error_message = BLocale::_($this->error_messages[self::IDEAL_XML_FAIL]);
            return false;
        }
