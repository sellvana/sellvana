...
/**
 * Class FCom_Promo_Model_PromoCartItem
 *
 * @property FCom_Promo_Main $FCom_Promo_Main
 * @property FCom_Catalog_Model_Product $FCom_Catalog_Model_Product
 */
class FCom_Promo_Model_PromoCartItem extends FCom_Core_Model_Abstract
{
    ...
    public function getItemRelatedPromos(FCom_Sales_Model_Cart_Item $item)
    {
        ...
        if (empty($promoCartItemCache[$cartId])) {
            $pcis = $this->orm('pci')->where('pci.cart_id', $item->get('cart_id'))
                ->join('FCom_Promo_Model_PromoCart', ['pc.id', '=', 'pci.promo_cart_id'], 'pc')
                ->join('FCom_Promo_Model_Promo', ['p.id', '=', 'pc.promo_id'], 'p')
                ->select('pc.*')
        ...
        if (!empty($skus)) {
            ...
            if (!empty($skus)) {
                ...
                $newProducts = $this->FCom_Catalog_Model_Product->orm()
