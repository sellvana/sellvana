          ...
          <div class="subpanel panel-attribute">
            ...
            <?php foreach ( $data as $obj ): ?>
                <?php if ( !empty( $s[ 'filter_selected' ][ $obj->key ] ) && in_array( $obj->name, $s[ 'filter_selected' ][ $obj->key ] ) ):?>
                    <li><a class="active" href="<?=BUtil::setUrlQuery( BRequest::currentUrl(), array( $obj->param => '' ) )?>"><span class="icon"></span><?=$obj->name?> <span class="badge">(<?=$obj->count?>)</span></a></li>
                    <?php if ( true == $s[ 'save_filter' ] ):?>
                        <input type="hidden" name="<?=$obj->param?>" value="<?=$obj->name?>" />
                    <?php endif; ?>
                <?php else :?>
                    <li><a href="<?=BUtil::setUrlQuery( BRequest::currentUrl(), array( $obj->param => $obj->name ) )?>"><span class="icon"></span><?=$obj->name?> <span class="badge">(<?=$obj->count?>)</span></a></li>
                <?php endif; ?>
            <?php endforeach ?>
            </ul>
          </div>
