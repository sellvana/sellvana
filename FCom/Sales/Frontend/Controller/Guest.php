...
/**
 * Class FCom_Sales_Frontend_Controller_Guest
 *
 * @property FCom_Sales_Main $FCom_Sales_Main
 * @property FCom_Sales_Model_Order $FCom_Sales_Model_Order
 * @property FCom_Customer_Model_Customer $FCom_Customer_Model_Customer
 */
class FCom_Sales_Frontend_Controller_Guest extends FCom_Frontend_Controller_Abstract
{
    ...
    public function action_create_account__POST()
    {
        try {
            ...
            $result = [];
            $this->FCom_Sales_Main->workflowAction('customerCreatesAccountFromOrder', [
                'order_id' => $orderId,
    ...
    public function action_merge_order__POST()
    {
        try {
            ...
            $result = [];
            $this->FCom_Sales_Main->workflowAction('customerMergesOrderToAccount', [
                'order_id' => $orderId,
                'post' => $this->BRequest->post(),
                'result' => &$result,
            ]);
    ...
    public function action_find_order__POST()
    {
        ...
        $order = false;
        $ok = $this->BLoginThrottle->init('FCom_Sales_Model_Order', $post['id']);
        if ($ok) {
            $order = $this->FCom_Sales_Model_Order->loadWhere([
                'unique_id' => $post['id'],
