...
class FCom_Admin_Model_User extends FCom_Core_Model_Abstract
{
    ...
    protected $_validationRules = array(
        ...
        array('email', '@email'),
        array('password', 'FCom_Admin_Model_User::validatePasswordRequirement', 'Password must be at least 7 characters in length and must include at least one letter, one capital letter, one number, and one special character.'),
    ...
    public static function validatePasswordRequirement($data, $args)
    {
        $password = $data[$args['field']];
        if(!preg_match('/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[~!@#$%^&*()_+=}{><;:\]\[?]).{7,}/', $password)) {
            return false;
        }
        return true;
    }
