...
/**
 * Class Sellvana_Blog_Admin_Controller_Post
 *
 ...
 */
class Sellvana_Blog_Admin_Controller_Post extends FCom_Admin_Controller_Abstract_GridForm
{
...
//                'href' => $this->BApp->href('blog/post/form/?id=:id')
    ...
    public function gridOrmConfig($orm)
    {
        ...
        if ($this->BSession->get('categoryBlogPost')) {
            $orm->join('Sellvana_Blog_Model_PostCategory', [$this->_mainTableAlias . '.id', '=', 'c.post_id'], 'c')
     ...
     * @param Sellvana_Blog_Model_PostCategory $model
    ...
    public function postGridConfig($model, $related = true)
    {
        ...
        $orm->join('FCom_Admin_Model_User', ['p.author_user_id', '=', 'u.id'], 'u')
            ->select_expr('CONCAT_WS(" ", u.firstname,u.lastname)', 'author');
        $orm->join('Sellvana_Blog_Model_PostCategory', ['p.id', '=', 'cp.post_id'], 'cp')
