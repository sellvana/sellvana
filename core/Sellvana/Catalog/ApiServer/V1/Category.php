...
/**
 * Class Sellvana_Catalog_ApiServer_V1_Category
 *
 * @property Sellvana_Catalog_Model_Category $Sellvana_Catalog_Model_Category
 */
class Sellvana_Catalog_ApiServer_V1_Category extends FCom_ApiServer_Controller_Abstract
{
    ...
    public function action_index()
    {
        ...
        if ($id) {
            $categories[] = $this->Sellvana_Catalog_Model_Category->load($id);
        } else {
            $categories = $this->Sellvana_Catalog_Model_Category->orm()->limit($len, $start)->find_many();
        }
        if (empty($categories)) {
            $this->ok();
        }
        $result = $this->Sellvana_Catalog_Model_Category->prepareApiData($categories);
        $this->ok($result);
    }
    ...
    public function action_index__POST()
    {
        ...
        if (!empty($post['parent_id'])) {
            $category = $this->Sellvana_Catalog_Model_Category->load($post['parent_id']);
        } else {
    ...
    public function action_index__PUT()
    {
        ...
        $category = $this->Sellvana_Catalog_Model_Category->load($id);
        if (!$category) {
