...
/**
 * Class Sellvana_Catalog_ApiServer_V1_Product
 *
 * @property Sellvana_Catalog_Model_Product $Sellvana_Catalog_Model_Product
 * @property Sellvana_Catalog_Model_CategoryProduct $Sellvana_Catalog_Model_CategoryProduct
 */
class Sellvana_Catalog_ApiServer_V1_Product extends FCom_ApiServer_Controller_Abstract
{
    public function action_index()
    {
        ...
        if ($id) {
            $products[] = $this->Sellvana_Catalog_Model_Product->load($id);
        } else {
            $products = $this->Sellvana_Catalog_Model_Product->orm()->limit($len, $start)->find_many();
        }
        if (empty($products)) {
            $this->ok();
        }
        $result = $this->Sellvana_Catalog_Model_Product->prepareApiData($products, true);
        $this->ok($result);
    }
    ...
    public function action_index__POST()
    {
        ...
        $data = $this->Sellvana_Catalog_Model_Product->formatApiPost($post);
        $product = false;
        try {
            $product = $this->Sellvana_Catalog_Model_Product->create($data)->save();
        } catch (Exception $e) {
            if (23000 == $e->getCode()) {
        ...
        if (!empty($post['categories_id'])) {
            ...
            foreach ($post['categories_id'] as $catId) {
                $this->Sellvana_Catalog_Model_CategoryProduct->create([
                    'category_id' => $catId,
