...
/**
 * Class Sellvana_Customer_ApiServer_V1_Customer
 *
 * @property Sellvana_Customer_Model_Customer $Sellvana_Customer_Model_Customer
 */
class Sellvana_Customer_ApiServer_V1_Customer extends FCom_ApiServer_Controller_Abstract
{
    public function action_index()
    {
        ...
        if ($id) {
            $customers[] = $this->Sellvana_Customer_Model_Customer->load($id);
        } else {
            $customers = $this->Sellvana_Customer_Model_Customer->orm()->limit($len, $start)->find_many();
        }
        if (empty($customers)) {
            $this->ok();
        }
        $result = $this->Sellvana_Customer_Model_Customer->prepareApiData($customers);
        $this->ok($result);
    }
    ...
    public function action_index__POST()
    {
        ...
        $data = $this->Sellvana_Customer_Model_Customer->formatApiPost($post);
    ...
    public function action_index__PUT()
    {
        ...
        $customer = $this->Sellvana_Customer_Model_Customer->load($id);
        if (!$customer) {
