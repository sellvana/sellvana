 ...
 */
class Sellvana_Promo_Frontend extends BClass
{
    ...
    {
        /* @var $cart Sellvana_Sales_Model_Cart */
        $cart = $args['model'];

        $items = $cart->items();
        if (!$items) {
            $allCartPromo = $this->Sellvana_Promo_Model_PromoCart->orm()->where('cart_id', $cart->id)->find_many();
        ...
        foreach ($items as $item) {
            /* @var $item Sellvana_Sales_Model_Cart_Item */
        ...
        $activePromo = [];
        $activePromoIds = [];
        $promoList = $this->Sellvana_Promo_Model_Promo->getActive();
        if (!$promoList) {
            return;
        }

        foreach ($promoList as $promo) {
            $promoProductsInGroup = $this->Sellvana_Promo_Model_Product->orm('p')
                    ->select('p.product_id')
                    ->select('p.group_id')
                    ->join($this->Sellvana_Promo_Model_Group->table(), "g.id = p.group_id", "g")
        ...
        $allCartItemPromo = $this->Sellvana_Sales_Model_Cart_Item->orm()->where('cart_id', $cart->id)
        ...
        if (!empty($activePromo)) {
            foreach ($activePromo as $promo) {
                $promoCart = $this->Sellvana_Promo_Model_PromoCart->orm()->where('cart_id', $cart->id)
                        ->where('promo_id', $promo->id)
                    ->find_one();
                if (!$promoCart) {
                    $promoCart = $this->Sellvana_Promo_Model_PromoCart->create(['cart_id' => $cart->id, 'promo_id' => $promo->id]);
                }
    ...
    public function onPromoCartAddProduct($args)
    {
        /* @var $cart Sellvana_Sales_Model_Cart */
        $cart = $args['model'];
        /* @var $currentItem Sellvana_Sales_Model_Cart_Item */
        ...
        foreach ($items as $item) {
            ...
            foreach ($promoIds as $promoId) {
                $promoList[$promoId] = $this->Sellvana_Promo_Model_Promo->load($promoId);
            }
        }
        ...
        foreach ($promoList as $promo) {
            ...
            if ($promo->get_type == 'qty') {
                ...
                if ($promo->get_group == 'any_group') {
                    ...
                    $item = $this->Sellvana_Sales_Model_Cart_Item->loadWhere([
                        'cart_id'      => $cart->id,
                    ...
                    if ($item && $promo->get_amount > $item->qty) {
                        ...
                        if ($currentItem->qty == 1) {
                            ...
                            $item = $this->Sellvana_Sales_Model_Cart_Item->create([
                                'cart_id'      => $cart->id,
                                'product_id'   => $currentItem->product_id,
                                'qty'          => 1,
                                'price'        => 0,
                ...
                if ($promo->get_group == 'same_group') {
                    ...
                    $groupProduct = $this->Sellvana_Promo_Model_Product->orm()->where('promo_id', $promo->id())
                    ...
                    foreach ($items as $item) {
                        ...
                        $groupProductItem = $this->Sellvana_Promo_Model_Product->orm()->where('promo_id', $promo->id())
                ...
                if ('diff_group' == $promo->get_group) {
                    ...
                    foreach ($items as $item) {
                        ...
                        $groupProductsBuy = $this->Sellvana_Promo_Model_Product->orm('p')
                            ->select('p.group_id')
                            ->join($this->Sellvana_Promo_Model_Group->table(), "g.id = p.group_id", "g")
                            ->where('p.promo_id', $promo->id())
                    ...
                    $groupProductGet = $this->Sellvana_Promo_Model_Product->orm('p')
                        ->select('p.group_id')
                        ->join($this->Sellvana_Promo_Model_Group->table(), "g.id = p.group_id", "g")
                        ->where('p.promo_id', $promo->id())
