...
/**
 * Class Sellvana_CustomField_Admin
 *
 ...
 */
class Sellvana_CustomField_Admin extends BClass
{
/*
...
*/
    ...
    {
        /** @var Sellvana_CustomField_Model_Field[] $fields */
        $fields = $this->Sellvana_CustomField_Model_Field->orm('f')->find_many();
        foreach ($fields as $f) {
            $col = ['label' => $f->field_name, 'index' => 'pcf.' . $f->field_name, 'hidden' => true];
            if ($f->admin_input_type == 'select') {
                $col['options'] = $this->Sellvana_CustomField_Model_FieldOption->orm()
                    ->where('field_id', $f->id)
    ...
    public function onProductsFormViewBefore()
    {
        ...
        $fieldsOptions = [];
        $fields = $this->Sellvana_CustomField_Model_ProductField->productFields($p);
        if ($fields) {
            $fieldIds = $this->BUtil->arrayToOptions($fields, 'id');
            $fieldOptionsAll = $this->Sellvana_CustomField_Model_FieldOption->orm()->where_in("field_id", $fieldIds)
    ...
    public function onProductFormPostAfterValidate($args)
    {
        ...
        $prodVarfieldHlp = $this->Sellvana_CustomField_Model_ProductVarfield;
        /** @var Sellvana_CustomField_Model_ProductVarfield[] $prodVarfieldModels */
        $prodVarfieldModels = $prodVarfieldHlp->orm()->where('product_id', $pId)->find_many_assoc('field_id');

        // retrieve product variant models
        $prodVariantHlp = $this->Sellvana_CustomField_Model_ProductVariant;
        ...
        /** @var Sellvana_CustomField_Model_Field[] $fieldModels */
        $fieldModels = [];
        $fieldsByCode = [];
        if ($varFieldsData) {
        ...
        if ($variantsData) {
            ...
            $fieldOptionHlp = $this->Sellvana_CustomField_Model_FieldOption;
            /** @var Sellvana_CustomField_Model_FieldOption[] $fieldOptionsModels */
            ...
            $prodVariantFieldHlp = $this->Sellvana_CustomField_Model_ProductVariantField;
            /** @var Sellvana_CustomField_Model_ProductVariantField[] $prodVariantFieldModels */
...
#echo "<pre>"; var_dump($variantsData); exit;
            ...
            $prodVariantImageHlp = $this->Sellvana_CustomField_Model_ProductVariantImage;
