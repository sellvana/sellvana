 ...
 */
class Sellvana_CustomField_Model_ProductField extends FCom_Core_Model_Abstract
{
    ...
    public function productFields($p, $r = [])
    {
        ...
        if ($p->get('_fieldset_ids') || !empty($r['add_fieldset_ids'])) {
            ...
            $where['OR'][] = "f.id IN (SELECT field_id FROM " . $this->Sellvana_CustomField_Model_SetField->table()
        ...
        if (!$where) {
            ...
            $fields = $this->Sellvana_CustomField_Model_Field->orm('f')
                    ->select("f.*")
                    ->left_outer_join($this->Sellvana_CustomField_Model_SetField->table(), 'f.id = sf.field_id', 'sf') //todo: implement override left_outer_join to IDE understand BORM::where
                    ->where($where, null)
    ...
    {
        $field = $this->Sellvana_CustomField_Model_Field->load($hide_field);
