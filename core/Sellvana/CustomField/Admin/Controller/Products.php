...
/**
 * Class Sellvana_CustomField_Admin_Controller_Products
 *
 ...
 */
class Sellvana_CustomField_Admin_Controller_Products extends FCom_Admin_Controller_Abstract
{
    /**
     * @return array
     */
    public function fieldsetsGridConfig()
    {
        $config = [
            'grid' => [
                ...
                'orm' => 'Sellvana_CustomField_Model_SetField',
                'columns' => [
    ...
    public function variantFieldGridConfig($model)
    {
        ...
        $varFields = $this->Sellvana_CustomField_Model_ProductVarfield->orm('vf')
        ...
        if ($varFields) {
            $varFieldsOptions = $this->Sellvana_CustomField_Model_FieldOption->orm()
...
#var_dump($columns); exit;
        ...
        $variants = $this->Sellvana_CustomField_Model_ProductVariant->orm()->where('product_id', $model->id())->find_many();
        $images = $this->Sellvana_CustomField_Model_ProductVariantImage->orm()->where('product_id', $model->id())->find_many();
        $invSkus = [];
        if ($variants !== null) {
    ...
    public function action_field_remove()
    {
        ...
        $this->Sellvana_CustomField_Model_ProductField->removeField($p, $hide_field);
        $this->BResponse->json('');
    }
    ...
    public function action_fields_partial()
    {
        ...
        $fields_options = [];
        $fields = $this->Sellvana_CustomField_Model_ProductField->productFields($p, $this->BRequest->request());
        foreach ($fields as $field) {
            $fields_options[$field->id()] = $this->Sellvana_CustomField_Model_FieldOption->orm()
    ...
    {
        $sets = $this->BDb->many_as_array($this->Sellvana_CustomField_Model_Set->orm('s')->select('s.*')->find_many());
    ...
    {
        $fields = $this->BDb->many_as_array($this->Sellvana_CustomField_Model_SetField->orm('s')->select('s.*')->find_many());
    ...
    public function action_get_fieldset()
    {
        $r = $this->BRequest;
        $id = $r->get('id');
        $set = $this->Sellvana_CustomField_Model_Set->load($id);
        $fields = $this->BDb->many_as_array($this->Sellvana_CustomField_Model_SetField->orm('sf')
        ...
        foreach ($fields as &$field) {
            if ($field['admin_input_type'] === 'select' ||  $field['admin_input_type'] === 'multiselect') {
                $field['options'] = $this->Sellvana_CustomField_Model_FieldOption->getListAssocById($field['id']);
            }
        }
    ...
    public function action_get_field()
    {
        $r = $this->BRequest;
        $id = $r->get('id');
        $field = $this->Sellvana_CustomField_Model_Field->load($id);
    ...
    public function action_save__POST()
    {
        try {
            ...
            $prodId = $data['id'];
            $json = $data['json'];
            $hlp = $this->Sellvana_CustomField_Model_ProductField;
    ...
    public function action_get_fields__POST()
    {
        try {
            ...
            $optionsHlp = $this->Sellvana_CustomField_Model_FieldOption;
            $fields = $this->Sellvana_CustomField_Model_Field->orm()->where('id', $ids)->find_many_assoc();
    ...
    {
        $f = $this->Sellvana_CustomField_Model_Field;
