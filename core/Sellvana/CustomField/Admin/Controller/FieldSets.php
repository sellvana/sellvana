...
/**
 * Class Sellvana_CustomField_Admin_Controller_FieldSets
 ...
 */
class Sellvana_CustomField_Admin_Controller_FieldSets extends FCom_Admin_Controller_Abstract
{
    protected $_permission = 'custom_fields';

    public function fieldSetsGridConfig()
    {
        $orm = $this->Sellvana_CustomField_Model_Set->orm('s')->select('s.*')
            ->select('(select count(*) from ' . $this->Sellvana_CustomField_Model_SetField->table() . ' where set_id=s.id)', 'num_fields');
...
//                    array('name' => '_actions', 'label' => 'Actions', 'sortable' => false, 'data' => array('delete' => 'noconfirm'))
    ...
    public function fieldsetModalAddGridConfig()
    {
        $config = [
            'config' => [
                ...
                'orm' => 'Sellvana_CustomField_Model_Field',
                'columns' => [
                    ['type' => 'row_select'],
                    ['name' => 'id', 'label' => 'ID', 'width' => 30],
    ...
    {
        $fld = $this->Sellvana_CustomField_Model_Field;
        $orm = $this->Sellvana_CustomField_Model_Field->orm('f')->select('f.*')
            ->select('(select count(*) from ' . $this->Sellvana_CustomField_Model_FieldOption->table() . ' where field_id=f.id)', 'num_options')
        ;
    ...
    {
        $orm = $this->Sellvana_CustomField_Model_SetField->orm('sf')
            ->join('Sellvana_CustomField_Model_Field', ['f.id', '=', 'sf.field_id'], 'f')
    ...
    public function action_fieldset_modal_add_grid_data()
    {
        ...
        $view = $this->view('core/backbonegrid');
        $orm = $this->Sellvana_CustomField_Model_Field->orm()->select('*');
    ...
    {
        $orm = $this->Sellvana_CustomField_Model_FieldOption->orm('fo')->select('fo.*')
    ...
    public function action_options()
    {
        ...
        $options = $this->Sellvana_CustomField_Model_FieldOption->getListAssocById($id);
    ...
    public function action_grid_data__POST()
    {
        ...
        switch ($r->post('oper')) {
            ...
            case 'edit':
                ...
                $set = $this->Sellvana_CustomField_Model_Set->load($data['id']);
                unset($data['id'], $data['oper'], $data['field_ids']);
                $set->set($data)->save();
                $result = $set->as_array();
                $result['num_fields'] = $num_fields;
                ...
                break;
            default:
                $this->_processGridDataPost('Sellvana_CustomField_Model_Set');
                break;
        }
    ...
    {
        //$this->_processPost('Sellvana_CustomField_Model_SetField', array('set_id'=>$this->BRequest->get('set_id')));
    ...
    public function action_field_grid_data__POST()
    {
        ...
        $this->_processGridDataPost('Sellvana_CustomField_Model_Field');
    }

    public function gridPostAfter($args)
    {
        if ($this->getAction() == 'field_grid_data') {
            /** @var Sellvana_CustomField_Model_Field $model */
            $data = $args['data'];
            $model = $args['model'];
            $hlp = $this->Sellvana_CustomField_Model_FieldOption;
            $op = 0;
    ...
    public function action_field_option_grid_data__POST()
    {
        ...
        $this->BResponse->json(['success' => true, 'options' => $op]);
        //$this->_processGridDataPost('Sellvana_CustomField_Model_FieldOption', array('field_id'=>$this->BRequest->get('field_id')));
    }
    ...
    public function action_form()
    {
        ...
        if ($id) {
            $model = $this->Sellvana_CustomField_Model_Set->load($id);
            if (empty($model)) {
            ...
            $model = $this->Sellvana_CustomField_Model_Set->create();
        }
    ...
    public function action_form__POST()
    {
        ...
        try {
            ...
            $this->BEvents->fire('Sellvana_CustomField_Admin_Controller_FieldSets::form_post',
    ...
    public function action_unique_field__POST()
    {
        ...
        try {
            ...
            $val = $p[$name];
            $exists = $this->Sellvana_CustomField_Model_Field->orm()->where($name, $val)->find_one();
    ...
    public function action_unique_set__POST()
    {
        ...
        try {
            ...
            $val = $p[$name];
            $exists = $this->Sellvana_CustomField_Model_Set->orm()->where($name, $val)->find_one();
            $result = ['unique' => !$exists, 'id' => !$exists ? -1 : $exists->id()];
        } catch (Exception $e) {
            $result = ['error' => $e->getMessage()];
        }
