...
class FCom_Core_Vendor_Embed extends BClass
{
        ...
        switch ($data->type) {
            case 'photo':
                ...
                $return = '<a href="' . $this->escUrl($url) . '"><img src="' . htmlspecialchars($data->url, ENT_QUOTES,
                        ...
                        'UTF-8') . '" width="' . htmlspecialchars($data->width, ENT_QUOTES,
                        'UTF-8') . '" height="' . htmlspecialchars($data->height, ENT_QUOTES, 'UTF-8') . '" /></a>';
                break;
            case 'video':
            case 'rich':
    ...
    function escUrl($url)
    {
         ...
         */
        if (strpos($url, ':') === false && !in_array($url[0], ['/', '#', '?']) && !preg_match('/^[a-z0-9-]+?\.php/i',
                $url)
        ) {
            $url = 'http://' . $url;
        }
