...
define(['underscore', 'react', 'select2', 'daterangepicker', 'datetimepicker'], function (_, React) {
    var FComFilter = React.createClass({
        ...
        render: function() {
            ...
            var filterNodes = _.map(filters, function(f, i) {
                ...
                return (<FComFilterNodeContainer filter={f} setFilter={that.doFilter} key={f.field} setStateFilter={that.setStateFilter} capitaliseFirstLetter={that.capitaliseFirstLetter} keepShowDropDown={that.keepShowDropDown} getConfig={that.props.getConfig} />);
            });
    ...
    var FComFilterText = React.createClass({
        ...
        render: function() {
            ...
            var operations = this.getOperations().map(function(item) {
                return ( <li key={item.op}> <a className="filter_op" data-id={item.op} onClick={that.setStateOperation} href="#">{item.name}</a> </li> )
            });
    ...
    var FComFilterDateRange = React.createClass({
        ...
        render: function() {
            ...
            var operations = this.getOperations().map(function(item) {
                return ( <li key={item.op}> <a className={"filter_op " + (item.range ? 'range' : 'not_range')} data-id={item.op} onClick={that.setStateOperation} href="#">{item.name}</a> </li> )
            });
