...
function print_country(country_id, country_name, state_id, state_name){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var option_str = document.getElementById(country_id);
	var x, i=0;
	for(x in country_arr){
            var selected = false;
            if (country_name == country_arr[x]) {
                selected = true;
                print_state(state_id, i, state_name);
            }
		option_str.options[i++] = new Option(country_arr[x],country_arr[x],selected);
	}
}
...
function print_state(state_id, state_index, state_name){
	...
	for(x in state_arr){
            var selected = false;
            if(state_name == state_arr[x]){
                selected = true;
            }
            option_str.options[i++] = new Option(state_arr[x],state_arr[x], selected);
	}
}
